	...
	bool SNMPRequest::parseFrom(unsigned char *buf)
	{
		...
		if (buf[0] != 0x30)
		{
			Snmp_Serial_printf("[DEBUG Request] Packet is not an SNMPRequest, expected 0x30, received: 0x%02x\n", buf[0]);
		...
		{
			Snmp_Serial_println(F("[DEBUG Request] SNMP packet too short, needs to be > 30"));
			return false;
		}
