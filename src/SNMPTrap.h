...
// it includes the main trapOID, other trap-ish options, and an attached list of the OIDCallback objects (the ones you get back from addIntegerHandler etc) to contain the values that need to be ent with this Trap
// it ca then be called from code, trap->send of fuckin whatever. trap receivers should also be attached to a list that can be changed
	...
	class SNMPTrap {
	  public:
	    ...
	    UDP* _udp = 0;
	    bool sendTo(IPAddress ip, uint16_t port=162)
	    {
	        if(!_udp)
	        {
	            return false;
	        ...
	        if(!build())
	        {
	            Snmp_Serial_println("[DEBUG Trap] Failed Building packet...");
	            delete packet;
	            packet = 0;
	            return false;
	        }
	        Snmp_Serial_println("[DEBUG Trap] Sending packet...");
